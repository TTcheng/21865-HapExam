...
public interface OrderService extends IBaseService<OrderSummaryVO>, ProxySelf<OrderService> {
    List<OrderSummaryVO> selectSummary(IRequest request,OrderSummaryVO orderSummaryVO, int page, int pagesize);

    List<OrderDetailVo> getDetail(IRequest request, OmOrderLines vo, int page, int pagesize);

    boolean save(OmOrderHeaders header, List<OmOrderLines> lines, IRequest request);
}
