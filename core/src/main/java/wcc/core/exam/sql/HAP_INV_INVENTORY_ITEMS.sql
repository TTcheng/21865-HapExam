...
CREATE TABLE `hap_inv_inventory_items` (
  ...
  `ITEM_CODE` varchar(60) CHARACTER SET utf8  NOT NULL COMMENT '物料编码',
  `ITEM_UOM` varchar(60) CHARACTER SET utf8 NOT NULL COMMENT '物料单位',
  `ITEM_DESCRIPTION` varchar(240) CHARACTER SET utf8  NOT NULL COMMENT '物料描述',
  `ORDER_FLAG` varchar(1) CHARACTER SET utf8  NOT NULL DEFAULT 'Y' COMMENT '是否能用于销售',
  `START_ACTIVE_DATE` datetime NULL COMMENT '生效起始时间',
  `END_ACTIVE_DATE` datetime NULL COMMENT '生效结束时间',
  `ENABLED_FLAG` varchar(1) CHARACTER SET utf8 NOT NULL DEFAULT 'Y' COMMENT '启用标识',
